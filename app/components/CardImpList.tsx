...
interface CardImpListProps {
  notionImpData: { title: string; updateUser: string, lastEditBy: String, icon: string, link: string; isActive: boolean }[];
  onCardClick: (link: string) => void; // 追加
}

const CardImpListProps: React.FC<CardImpListProps> = ({ notionImpData, onCardClick }) => {
    return (
      <div className="grid text-center lg:max-w-5xl lg:w-full lg:grid-cols-3 lg:text-left gap-4">
        {notionImpData.map((item, index) => (
          <Card
            key={index}
...
export default CardImpListProps;
